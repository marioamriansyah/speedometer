...
//Radio Player
const playButton = document.getElementById('playRadio');
const pauseButton = document.getElementById('pauseRadio');
const radioAudio = document.getElementById('radioAudio');
const volumeControl = document.getElementById('volumeControl');
const volumeSlider = document.getElementById('volumeSlider');
const volumeValue = document.getElementById('value');

let val = 0.10; // default volume
let oldval, startY, height;
...
function updateVolumeDisplay(vol) {
    volumeSlider.style.setProperty('--val', vol);
...
volumeSlider.addEventListener('pointerdown', (ev) => {
    oldval = val;
    startY = ev.clientY;
    height = volumeSlider.clientHeight;

    volumeSlider.addEventListener('pointermove', slide);
    volumeSlider.setPointerCapture(ev.pointerId);
});

volumeSlider.addEventListener('pointerup', (ev) => {
    volumeSlider.releasePointerCapture(ev.pointerId);
    volumeSlider.removeEventListener('pointermove', slide);
});
